...
export function diffFiles(baseRef, headRef) {
	return execFileSync('git', ['diff', '--name-only', baseRef, headRef], {
		cwd: repoRoot,
		encoding: 'utf8'
	})
		.split('\n')
...
// package name -> changed shipped files. The changeset guard and the Dependabot
// changeset author must agree on this set exactly: if the author writes a
// changeset for a different set than the guard demands, the PR deadlocks.
export function touchedPublishablePackages(changedFiles) {
...
// Imported for its exports by changed-publishable-packages.mjs — argv parsing
// and process.exit below must not run on import.
...
function runGuard() {
	...
	const touched = touchedPublishablePackages(changedFiles);
