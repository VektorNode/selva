...
if [ "$SKIP_PM2" = false ]; then
  ...
  # Runtime config is loaded from .env via PM2's env_file — any var added there
  # (including provider-specific ones like SUPABASE_URL) flows through without
  # touching this script. NODE_ENV is the only thing pinned here.
  cat > "$CONFIG_FILE" << EOF
// PM2 process file — auto-generated by setup.sh on $(date).
// Runtime config is loaded from packages/compute-app/.env via env_file below.
module.exports = {
	apps: [
		{
			name: 'selva-compute',
			...
			cwd: '$INSTALL_DIR/packages/compute-app',
			env_file: '$ENV_FILE',
			env: {
				NODE_ENV: 'production'
			}
