...
public class SchemaValidator
{
	...
	{
		if (schema == null) return ValidationResult.Failure("Schema is null");
	...
	{
		if (schema.Inputs == null || schema.Outputs == null) return; // Already flagged in basic structure validation
	...
	{
		if (schema.Layout == null) return; // Already flagged in basic structure validation
		...
		if (schema.Layout is TabbedLayoutConfig tabbedLayout)
		{
			...
			foreach (var tab in tabbedLayout.Tabs)
			{
				...
				foreach (var group in tab.Groups) ValidateGroup(group, inputParamIds, outputParamIds, layoutItemIds, issues);
			}
		...
		List<ValidationIssue> issues)
	{
		...
		// Validate each item
		foreach (var item in group.Items) ValidateLayoutItem(item, inputParamIds, outputParamIds, layoutItemIds, issues);
	}
