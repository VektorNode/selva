...
// A DrawingView with no Scale/Size/Length auto-fits to its layout context. Stack used to hand
// children an infinite main axis, so a view only fitted its cross axis and overflowed the page
// along the other. These tests pin "auto-fit fits the box you're actually in" at any nesting depth.
public class StackDrawingViewFitTests
{
	...
	// DrawingView can't split, so a tall auto-fit view has to get its height right up front —
	// pagination can't rescue an overflow here the way it can for splittable content.
	[Fact]
	...
	// A horizontal stack never handed down a width at all — clamping inside DrawingView alone
	// couldn't fix that, since there was no width to clamp against.
	[Fact]
	...
	public void Explicit_scale_is_not_overridden_by_the_stack_budget()
	{
		...
		// Compare the three against each other, not a literal: the resolved width includes the
		// view's frame padding and stroke on top of the 100mm of geometry.
	...
	// Grid's Pass 1 used to measure cells with an empty context, so an auto-fit view reported
	// an unbounded natural size and that became the Auto track's size — nothing clamps an
	// Auto track afterwards, since it's sized to fit its content by definition.
	[Fact]
	...
	// GH_Page wraps a single content branch in a GroupElement, not a Stack. Group is a
	// primitive, so pagination used to measure it without resolving it — the DrawingViews
	// inside never saw the page context and kept their natural (oversized) size.
	[Fact]
	...
	{
		// NormalizeToOrigin wraps the branch in a transformed Group — the shape that actually
		// reaches pagination.
		var branch = new GroupElement
		{
