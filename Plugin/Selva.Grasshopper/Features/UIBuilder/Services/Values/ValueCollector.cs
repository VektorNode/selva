...
using Selva.Grasshopper.Features.Display.Services;
using Selva.Grasshopper.Features.FileIO.Services;

namespace Selva.Grasshopper.Features.UIBuilder.Services.Values;
...
public class ValueCollector
{
  ...
  private object ExtractValueListValue(GetValueListParameter valueListParam)
  {
    ...
    if (valueData != null && !valueData.IsEmpty)
    {
      ...
      if (allData.Count == 1)
      {
        return ExtractKeyFromValueListData(allData[0]);
      }

      if (allData.Count > 1)
      {
        return allData.Select(d => ExtractKeyFromValueListData(d)).ToList();
      }
    }
  ...
  {
    if (data is GH_ValueListDataGoo valueListData)
    {
