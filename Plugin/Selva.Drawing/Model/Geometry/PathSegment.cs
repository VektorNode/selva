...
// deliberate (matches what SVG paths and PDF content streams natively express).
public abstract record PathSegment
{
	...
	// small. The convenience helper FromQuadratic does the standard P1/P2 elevation.
	public sealed record CubicTo(Point2D Control1, Point2D Control2, Point2D To) : PathSegment
	{
		...
		{
			// Standard quadratic-to-cubic elevation:
	...
	// Elliptical arc command, mirroring SVG's A/a primitive — we keep the same parameters
	// because they round-trip cleanly to both SVG and PDF (PDF flattens to cubics; SVG
	// emits the arc directly).
