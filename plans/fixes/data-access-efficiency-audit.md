...
**Started:** 2026-07-05 (branch `beta`) · **Last full re-verification:** 2026-07-16 — every open item below was re-checked line-by-line against current source. Shipped 2026-07-17: B5's memory half (surviving half re-scoped as B5-lb) and O4 (server path; browser/CLI scope recorded as O4-scope). Q6 was widened after O4 surfaced a second flaky file. Shipped 2026-07-18: Q5's last item (core-loop E2E) — Q5 closed, P1 empty. 2026-07-31: solve-path re-verification against HEAD — single-flight coalescing now unconditional (closed), C11 filed, and **research question 4's "~6.6 solves/sec" figure corrected — it ignored the trailing-edge debounce and the single-in-flight throttle**. The drag-verification doc was then [retired unrun](../archive/verify-slider-drag-solve-path.md), its two live-stack claims folded into C4 as explicitly unverified. **C4's read count has never been measured** — do not promote it on the number alone. **Later the same day the rate-limit half was re-examined by simulation and the retirement note's conclusion was itself found wrong (the third error on this path): a slider drag _does_ hit the 429, across a wide band of ordinary move spacings. C4's rate-limit half raised to P2 — see the table in §C.**
...
| ☐      | **C4**    | 5–7 sequential DB reads per solve (⚠️ count unverified); **drag DOES 429 (simulated) — raised P2**; plus a false rate-limit comment | [§C](#c-solve-cycle)                |
...
3. **Draft-channel caching (→ C6):** should drafts get a short-TTL cache rather than a bespoke envelope cache in front of L1? _(Originally phrased as "a short-TTL L2"; the L2 tier has since been deleted — any answer here has to be an L1-level or `ISolveResultCache`-backed design, not a revival of it.)_
4. ~~**Does a real slider drag cost what we think?**~~ **CLOSED UNMEASURED 2026-07-31 — decision recorded, not a finding.** The dedicated verification doc was retired without ever being run and is [archived](../archive/verify-slider-drag-solve-path.md); its two surviving claims are folded into **C4** as explicit caveats. **The measurement was never performed** — no `pg_stat_statements` diff, no browser drag against a real stack. We chose to stop carrying a doc nobody was going to action rather than let it sit looking like a pending check. What the exercise produced, in order of what actually mattered:
   - _Single-flight inert unless a solve cache was mounted_ — **fixed in code**: the gate is gone, coalescing runs for every solve, and the `SOLVE_CACHE_PROVIDER` var it keyed off was deleted with the L2 tier.
   - _"The 150ms debounce permits ~6.6 solves/sec"_ — **our own error, and it was the number that made the whole thing look urgent.** The debounce is trailing-edge, so a continuous drag emits nothing until the user pauses; the client throttle is single-in-flight with one latest-wins slot, so requests cannot overlap. Real ceiling is `min(1/0.150, 1/solveRTT)` per second — natural dragging sits far below the 1.2/sec limit. **This error lived for two weeks inside the document written to warn against exactly this failure mode**: deriving a rate from one constant without checking what bounds it downstream. Two static analyses of this path, two errors. Weight code-reading claims about the solve path accordingly.
   - _~8 uncached DB reads per solve_ — still unverified; carried into C4 with a caveat marker.
   - _`incrementSolveCount` unbatched_ — true by reading, needs no measurement; moved to C11.
...
- **C4 — Hot-path reads per solve. [P2 — ⚠️ read count is STATIC, never measured]** **5 sequential awaited DB reads** on the common logged-in path, 7 with a share link: `tryResolveShareToken` (`:140`), `definitions.get` (`:201`), `projects.getProject` (`:209`), access check (`:217`/`:222`), `definitions.getVersion` (`:232`), `tryIncrementSolveCount` (`:274`), `resolveServerForOrg` (`:285`) — the last of which fans out to a 4-query cluster in `SupabaseComputeServerStore.getConfig` (`:79-93`), which is where the "~8 reads per solve" figure comes from. Bytes and warm clients are cached; the records are not — so a same-definition slider re-solve pays all of them. (`resolveServerForOrg` is now two cheap reads rather than a decrypt-everything scan — see 4b in git history — but it is still uncached.)

  **Verification caveat (folded in 2026-07-31 when the drag-measurement issue was retired unrun).** Every number above is from reading code. Nobody has diffed `pg_stat_statements` across a real drag, so the count could be wrong in either direction — Supabase may collapse some of these, or a read may fan out further than the call site suggests. **This matters for priority, not validity:** request-scoped memoization of records that cannot change within one request is correct regardless of whether the count is 5 or 8. But do not promote C4 above other P2 work on the strength of the number alone. **B9 is the standing cautionary tale** — ranked P1 off exactly this kind of static reasoning, and ~5th on the path once actually traced. If you touch this, measure first; it is one `pg_stat_statements` diff across one drag.

  **Related — can a drag hit the rate limit? YES, and more easily than two previous analyses concluded. [raised to P2, 2026-07-31]** The per-user bucket allows **1.2 solves/sec sustained** (`COMPUTE_RATE_LIMIT_MAX=120` / `COMPUTE_RATE_LIMIT_WINDOW_MS=100_000`, `packages/server/src/compute/limits.ts:280-281`; keyed `user:${id}` at `+server.ts:170`, so one user scrubbing alone can trip it).

  Two earlier readings of this path were both wrong, in opposite directions. The first claimed the 150ms debounce permits ~6.6/sec (ignored trailing-edge semantics). The second — the correction — claimed the single-in-flight throttle bounds the rate to `1/solveRTT` so "natural dragging sits far below 1.2/sec", and downgraded this to a pathological-input curiosity. **That correction is also wrong: the throttle bounds _concurrency_, not rate.** It drops superseded values, but every debounce fire landing while nothing is in flight starts a request immediately.

  Simulating the real composition (trailing debounce 150ms → single-in-flight latest-wins → 16-entry LRU memo → network), against 120/100s:

  | Drag style                                   | Network requests | Peak/100s | 429?    |
  | -------------------------------------------- | ---------------- | --------- | ------- |
  | Continuous scrub, 16ms frames, 100s          | **1**            | 1         | no      |
  | Discrete moves @160ms, 50ms solve            | 626              | 625       | **yes** |
  | Discrete moves @**500ms**, 50ms solve        | 201              | 200       | **yes** |
  | Discrete moves, values revisited (memo hits) | **1**            | 1         | no      |
  - **The 429 window is wide, not narrow:** any move spacing from ~150ms to ~830ms trips it. That is not a pathological pattern — it is someone nudging a slider once or twice a second to tune a value, which is the primary use of a slider.
  - **The throttle only rescues slow definitions:** sweeping RTT, the 429 persists until solve RTT ≈ **833ms**. A 500ms solve yields ~2/sec — still 167% of the limit.
  - **Time to first 429 is ~19s**, not ~100s. 120 requests at ~6/sec exhausts the budget in a fifth of the window.
  - **What actually protects normal use is the LRU memo, not the throttle** — and neither prior analysis mentioned it. Continuous scrubbing issues 1 request (debounce coalesces); revisiting values issues 1 (memo hits). Monotonic tuning through fresh values defeats both.
  - **User-visible failure is not a silent retry:** a 429 arms a client cooldown (`packages/selva/src/routes/library/[guid]/+page.svelte:30,49-52`) that short-circuits solves and shows `Rate limit reached. Try again in Ns.` mid-tuning.

  **Status: confirmed by executable simulation of the real code, still unconfirmed against a live stack.** The model could be wrong the way the prose was — but it is the first reproducible analysis of four, and it inverts the prior priority. Full derivation in the [retired verification doc](../archive/verify-slider-drag-solve-path.md). **Do not downgrade this again from a reading of the constants.** The fix is a product call (raise the limit, lengthen the debounce, or stop counting slider solves against the same bucket), not a patch.
...
- **C11 — `incrementSolveCount` is one unbatched RPC per successful solve. [P3 — moved out of the drag-verification issue 2026-07-31]** `api/compute/+server.ts:434` fires `increment_run_count` (`SupabaseDefinitionStore.ts:220`) per successful solve, fire-and-forget but still one Postgres round-trip each. It backs a display counter ("N runs") — it does not need to be transactional per solve, and `SupabaseSolveMetricSink` already has the buffering pattern to copy (~10 lines). **Deliberately parked, not blocked:** this is structurally identical to B9, which was ranked P1 off exactly this reasoning and landed ~5th on the path once traced. The drag measurement that would have ranked it was [retired unrun](../archive/verify-slider-drag-solve-path.md), so nothing is coming to justify it — ship it when someone is already in this file for another reason, not on its own. Note it shares the counter-write path with 4d (profile mutations → single RPC), so the two may merge.
