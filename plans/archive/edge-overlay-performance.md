...
> **Status: CLOSED (2026-07-31). Phases 0–3 shipped 2026-07-22; GPU-visual
> verification done 2026-07-31 — see "GPU-visual verification" at the bottom.**
> Phase 4 (Rhino-authored edges) was never residue of this plan: it is an optional
> fidelity feature on its own track, unstarted, and gets its own plan if pursued.
> See "Results" for the after-benchmarks.
> Scope, as written: `packages/compute` visualization (edges.ts, edge-extract.ts,
...
> **Those paths are historical.** The 2026-07-30
> [visualization-package](./visualization-package.md) move relocated all of it to
> `@selvajs/visualization`: `render/edges/*`, `render/edge-extract.ts`,
> `render/edge-detection-pass.ts`, `render/render-pipeline.ts`,
> `render/scene-setup/*` (`three-initializer` split into `init-three.ts` +
> `defaults.ts` + `pipeline-controller.ts`). Read old paths in this file as
> pointers to those.
...
- ~~Visual check of `EdgeDetectionPass` on a GPU (`pnpm example`), including the
  perspective↔ortho toggle and threshold tuning against real solves.~~ **Done
  2026-07-31 — see "GPU-visual verification" below.**
- Phase 4 (Rhino-authored edges) unchanged — still the fidelity ceiling, still
  a separate track. Not residue of this plan; if pursued, it gets its own plan
  (assessment above stands: binary per-mesh channel, opt-in, Brep-edges first).

---

## GPU-visual verification (2026-07-31) — closes the last open item

Run in headed Chrome on a real GPU (`ANGLE / NVIDIA RTX 3060 / D3D11`) against
`pnpm example`, driving the demo viewer through Playwright.

**The harness had to be repaired first.** `examples/` still lived in
`packages/compute` and imported `@/features/visualization/threejs/*` — paths
deleted by the 2026-07-30 package move, so `pnpm example` had been dead since.
Six of its eight `@/` imports pointed at a directory that no longer existed.
The tree moved to `packages/visualization/examples` (where `three` actually is —
compute must not depend on it), imports now target the public barrels
(`@/render`, `@/parse`), and `examples/**` joined the package tsconfig so a
rename breaks `pnpm type-check` instead of rotting silently. Two demo bugs fell
out: both toggled edges off with a bare `removeEdges` (the exact trap Phase 3
documented — it strips overlays but leaves the screen-space pass running; now
`viewer.clearEdges`), and `dmf-viewer` referenced a `DFF.dmf` fixture that does
not exist in the repo (now `test_mesh.dmf`).

**Verified:**

| Check                                                | Result                                                                                                                                        |
| ---------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| Crease look, line-overlay path (under cap)           | Correct — box/cylinder creases + silhouettes; torus correctly bare at 44°                                                                     |
| Per-mesh triangle cap                                | Engages, tags `userData.edgesSkipped = 'triangle-cap'`, no overlay                                                                            |
| Screen-space fallback above the cap                  | Engages — collapses to a single fullscreen composer pass (`calls: 1`) vs `calls: 5` forward with it off; draws creases the overlay path can't |
| `clearEdges`                                         | Removes overlays **and** stands the pass down                                                                                                 |
| perspective ↔ orthographic toggle                    | Pass stays correct across both and back; ortho gives the clean CAD wireframe look                                                             |
| Edge-cache plateau across solves (caching-audit §F1) | **Flat** — `rendererGeometries` 20 across 8 consecutive solves reusing one geometry                                                           |

The cap is **per-mesh**, not scene-total — worth stating because it is easy to
mis-test: six 900k-triangle meshes (5.4M total) correctly do _not_ trip a 4M cap.

**Found a real bug, now fixed** (`.changeset/edges-config-passthrough.md`):
`initThree`'s `edges.maxTriangles`, `edges.maxSegments` and
`edges.screenSpaceFallback` were silently dropped. All three are documented on
`EdgesConfig` and read at runtime — `applyEdges` forwards the caps to
`addEdgesAsync`, `updateEdgeFallback` gates the pass on `screenSpaceFallback` —
but `applyDefaults` never copied them out of the caller's options. So the
triangle cap was pinned to its 4M default (un-tunable in either direction) and
`screenSpaceFallback: false` did nothing. It type-checked because
`ResolvedOptions` is `Required<>` only at the top level, leaving `edges` with
`EdgesConfig`'s optional members. The fields now pass through as `undefined`
when unset, so `resolveOptions` in `edges/options.ts` remains the single owner
of the 4M/2M defaults; a regression test asserts every `EdgesConfig` key
survives resolution.

This is precisely the class of defect static tests cannot reach: every unit test
called the edge functions directly with explicit options, so none exercised the
`initThree` → `applyDefaults` → `applyEdges` path where the options were lost.
