...
<div
  class={`flex gap-1 border-b-2 border-gray-200 bg-gray-50 overflow-x-auto ${className}`}
>
  {#each tabs as tab}
    <div class="flex items-center">
      ...
      {#if editable && onRemove && tabs.length > 1}
        <button
          class="ml-2 text-gray-400 hover:text-red-600 transition-colors text-xl leading-none"
          onclick={(e) => {
            e.stopPropagation();
            onRemove?.(tab.id);
          }}
