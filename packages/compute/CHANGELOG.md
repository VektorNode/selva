...
- 4512068: Prune the public API and unwrap the package's internal layout. Every removed symbol has a
...
- 4512068: **`@selvajs/compute/core` is now backend-agnostic** — it names no Rhino concept, so a second
...
- 4512068: `ProcessedFile` keeps the GH-owned `subFolder` and `metadata` that `FileData` carried.
