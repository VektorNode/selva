 ...
 * the root path `{}`. Single source of truth for the branch-path shape
 * (`DataTreePath`); anything testing "does this string name a branch?" should
 * use this rather than re-inlining the regex.
 *
 ...
 * branch paths, each mapping to an array of values. Both the input-type
 * parsers (to pass a tree-access default through untouched) and `TreeBuilder`
 * (to dispatch it to `fromDataTreeDefault`) use this one predicate, so the two
