...
export interface GrasshopperDefinitionSource {
	...
	algo?: string | null;
	/** URL pointer to the definition file. */
	pointer?: string | null;
	filename?: string | null;
}

/**
 * Configuration for Grasshopper compute operations: server config plus
 * optional Grasshopper-specific computation settings.
 *
 * The definition source (pointer/algo) is not part of config. Pass the
 * definition directly to methods like `solve()`, `getIO()`, etc.
 */
export interface GrasshopperComputeConfig extends ComputeConfig {
	...
	dataversion?: 7 | 8 | null;
	cachesolve?: boolean | null;
	/** See {@link GrasshopperBaseSchema.cacheerroredsolves}. Only meaningful with `cachesolve`. */
	cacheerroredsolves?: boolean | null;
}
...
// Arguments sent to the Grasshopper compute endpoint: config options + definition source + input values.
...
/**
 * Response from the Grasshopper compute server: schema fields + computed results.
 *
...
export interface InputParamSchema {
	...
	stepSize?: number;
	default: any;
	/** Key-value pairs for dropdown (ValueList) options. */
	values?: Record<string, string>;
	/** Accepted file formats for a File input. */
