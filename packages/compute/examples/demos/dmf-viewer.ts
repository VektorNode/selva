 ...
 *
 *   1. Parse — `parseMeshBatchBlob(blob, { mergeByMaterial: false, applyTransforms: true })`, the
...
import { parseMeshBatchBlob } from '@/features/visualization/webdisplay/batch-parser';
import { updateScene } from '@/features/visualization/threejs/three-helpers';
import { removeEdges } from '@/features/visualization/threejs/edges';
import type { ThreeInitializerOptions, Look } from '@/features/visualization/types';

// Bundled sample DMFs from the repo's fixtures (outside the examples root; Vite serves them via ?url).
import sampleSmallUrl from '../../fixtures/test_file.dmf?url';
import sampleBuildingUrl from '../../fixtures/DFF.dmf?url';
	...
	try {
		...
		pg.setStatus(
			`Loaded ${meshes.length} mesh(es), ${tris} tris — ${name}\n` +
				`size: ${size.toArray().map((v) => v.toFixed(1)).join(' × ')}\n` +
...
pg.addButton('Sample: DFF.dmf', () => void loadDmfUrl(sampleBuildingUrl, 'DFF.dmf'));
