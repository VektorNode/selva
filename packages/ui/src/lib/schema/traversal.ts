...
//
// Readers are defensive: a missing layout or missing groups/items yields an empty
// result rather than throwing, so a malformed schema can't crash a consumer.

import type { UISchema, GroupConfig, LayoutItem } from '@selvajs/schemas';

/** Input layout items — excludes outputs and linebreaks. Carry `paramId` and `source`. */
export type InputLayoutItem = Extract<LayoutItem, { type: 'input' }>;
