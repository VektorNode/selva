...
- ff73858: Fix the scene panel crashing with `each_key_duplicate` on scenes holding duplicate object identities
...
- f24ae8d: Wire up merged-mesh picking, the new looks, and outliner expand/collapse-all
...
- 4e24b20: Follow the mesh identity rename: metadata dialogs exclude `id`, not `sourceComponentId`/`originalIndex`
...
- 6fa6b27: Three small fixes found in the open-issue backlog.
...
- fe4cbec: Make the viewer's branding logo watermark larger so it stays legible.
...
- 3e784c6: Expose the edge overlay through `ViewerConfig`.
...
- 4512068: Make the reported `SolveResult` reachable from a `ComputeApp` host, so a commit path can pair the
...
- 4512068: **`Sub Folder` now names the download archive, so one output can produce several zips.** This
...
- 4512068: `onViewerReady` on `<Viewer>` and `<ComputeApp>` hands the live three.js viewer to the host, so an
...
- 4512068: Drop the `rhino3dm` dependency. Curves now arrive pre-tessellated from the plugin, so the demo's
...
- 4512068: Fix a type error in `Viewer.svelte`: its `onMeshMetadataClicked` handler declared its parameter as
