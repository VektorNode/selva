...
<div class="gap-2 flex flex-col">
	...
	{:else if isNumberDisplay(item)}
		<div
			class="rounded px-3 py-3 font-mono text-sm min-h-[50px] border border-border bg-muted wrap-break-word"
		>
	...
	{:else if isTextDisplay(item)}
		<div class="group relative">
			{#if typeof value === 'object' && value !== null}
				<pre
					class="overflow-wrap-anywhere rounded px-3 py-3 font-mono text-sm max-h-96 min-h-[50px] overflow-auto border border-border bg-muted text-foreground">{formatValue(
						value
					)}</pre>
			{:else}
				<div
					class="overflow-wrap-anywhere rounded px-3 py-3 font-mono text-sm min-h-[50px] border border-border bg-muted wrap-break-word whitespace-pre-wrap text-foreground"
				>
					{#if value !== null && value !== undefined}
						{value}
					{:else}
			...
			{#if value !== null && value !== undefined}
				<Button
					...
					class="top-2 right-2 absolute transition-opacity {copied
						? 'opacity-100'
