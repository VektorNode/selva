...
function applyDefaults(options: ThreeInitializerOptions): Required<ThreeInitializerOptions> {
  ...
  // TODO: Check if thats actually the case (METERS)
  // Note: All Rhino geometry comes in METERS, sceneScale just changes the viewing perspective
  const scaleDefaults = {
    mm: {
...
function setupEnvironment(
  ...
  if (config.environment.enableEnvironmentLighting) {
    new RGBELoader().load(
      ...
      undefined,
      function (error) {
        console.warn('HDR texture could not be loaded, falling back to basic lighting:', error);
...
function setupEventHandlers(
  ...
  const fitToView = () => {
    ...
    if (box.isEmpty()) {
      console.warn('No objects to fit to view');
      return;
    }
