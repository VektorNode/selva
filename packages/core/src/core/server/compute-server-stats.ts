...
export default class ComputeServerStats {
  ...
  public async isServerOnline(): Promise<boolean> {
    ...
    try {
      ...
      console.info('[ComputeServerStats] Fetch error <-', err);
      return false;
    }
  }
  ...
  public async getActiveChildren(): Promise<number | null> {
    ...
    try {
      ...
      if (!response.ok) {
        console.warn('[ComputeServerStats] Failed to fetch active children:', response.status);
        return null;
      }
      ...
      if (isNaN(count)) {
        console.warn('[ComputeServerStats] Invalid active children response:', text);
        return null;
      }

      return count;
    } catch (err) {
      console.warn('[ComputeServerStats] Error fetching active children:', err);
      return null;
    }
  }
  ...
  public async getVersion(): Promise<{
    ...
    try {
      ...
      if (!response.ok) {
        console.warn('[ComputeServerStats] Failed to fetch version:', response.status);
        return null;
      }
      ...
      console.warn('[ComputeServerStats] Error fetching version:', err);
      return null;
    }
  }
  ...
  private ensureNotDisposed(): void {
    if (this.disposed) {
      throw new Error('ComputeServerStats has been disposed and cannot be used');
    }
  }
}
