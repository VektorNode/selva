...
import type { ComputeServerConfig, RequestContext, SolveFailureKind } from '@selvajs/platform';
import {
	resolveServerForOrg,
...
// Concise cache/timing logs (Selva cache hits, server decode/solve/encode). Safe
// to leave on — a couple of short lines per solve, no payloads.
...
// geometry) — very noisy. Separate opt-in flag so the cache logs above aren't
// drowned out. Only enable when you need raw request/response bodies.
const COMPUTE_DEBUG_VERBOSE = ['true', '1', 'yes'].includes(
...
// Per-server client+scheduler cache. Keyed by serverUrl+apiKey so multi-org
// BYO-compute deployments keep a warm client (and its solve dedup cache) for
// EACH server instead of thrashing a single slot — interleaved requests to two
// org servers used to dispose+recreate the scheduler on every call, discarding
// the dedup cache each time. AbortSignal still kills orphan solves on client
// disconnect. Bounded so a churn of one-off servers can't grow unbounded.
...
async function getClient(serverConfig: ComputeServerConfig): Promise<CachedClient> {
	...
	// the route reads them right after each solve resolves.
	...
	const scheduler = client.createScheduler({
		...
		// Observe the Selva in-process response cache. `fromCache === true` means
		// this solve was served from Selva's own cache — Rhino.Compute was never
		// called (you'll see NO [Compute/timing] line for it). `false` means it went
		// to the compute server. Always registered so the cache verdicts reach the
		// browser via Server-Timing; console logs stay debug-gated.
...
export const POST: RequestHandler = async ({ request, locals, url }) => {
	...
	try {
		...
		// DEBUG (SELVA_FLAG_COMPUTE_DEBUG): size of the solve result returned to the
		// browser. `serialized.length` is the JSON byte count (1 byte/char for the
		...
		if (COMPUTE_DEBUG) {
			// The server-side overhead the solve metric's `durationMs` doesn't capture.
			// `load` = auth + DB + definition fetch; `tree` = input tree build; `serialize`
			// = JSON.stringify of the result. The solve itself is timed separately (see the
			// solve metric's durationMs and the [Compute/selva-cache] line).
			console.log(
