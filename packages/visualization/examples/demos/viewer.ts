 ...
 * live — toggled on/off here via `setAmbientOcclusion`, `applyEdges`, and `clearEdges`.
 */
...
pg.addButton('Add Box', () =>
	addMesh(new THREE.BoxGeometry(1, 1, 1), `Box_${meshCount}`, Math.random())
);
...
// ── Edges (live applyEdges / clearEdges) ──────────────────────────────────────
// Toggling off goes through `clearEdges`, not a bare `removeEdges`: above the triangle cap the
// viewer swaps the overlay for the screen-space edge pass, and only `clearEdges` stands that pass
// down too. `removeEdges` alone would strip the line overlays and leave the fallback rendering.
pg.addSection('Edges');
