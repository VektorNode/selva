...
/**
 * Reads/writes compute.config.json. The file is re-read on every
 * `getConfig()` call so changes take effect without a restart.
 *
 * `ctx` is part of the contract but unused — there is no tenant scope to
 * enforce against the filesystem. A multi-tenant adapter would authorize
 * writes here.
 */
