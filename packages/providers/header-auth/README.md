...
Forward-auth implementation of `@selvajs/platform`'s `IAuthProvider`. Trusts identity headers set by an upstream reverse proxy (Caddy `forward_auth`, oauth2-proxy, Authelia, Pomerium, Traefik forward-auth, …).

**Auth-only.** No data store, no storage, no permissions. Pair it with whatever data provider you want — `@selvajs/local-provider` for filesystem JSON, `@selvajs/supabase-provider` for Postgres.

**Operator setup — a full Entra + oauth2-proxy + Caddy walkthrough, with troubleshooting:** [docs/self-hosting/providers/header-auth-entra.md](../../../docs/self-hosting/providers/header-auth-entra.md).
...
Run 1 and 2 from outside — from the host itself the direct hit reaches
loopback and passes even with the firewall wide open. If any of these
don't behave as expected, **do not put the deployment in front of real
users until they do.**
...
the user's first visit through the proxy, `identifyFromHeaders` matches the UPN
against the allowlist, fills in `email` / `displayName` from the matching
headers, and the user is authenticated. On subsequent visits the same lookup
runs — no session cookie, no token, no state in the browser beyond what the
proxy already has.
...
| File                                                   | Holds                                                                                                            |
| ------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------- |
| [src/HeaderAuthProvider.ts](src/HeaderAuthProvider.ts) | `HeaderAuthProvider` (`IAuthProvider`) and the internal `HeaderProxyAuth` (`IProxyAuth`, `identifyFromHeaders`). |
| [src/users.ts](src/users.ts)                           | `AllowlistStore`, `AllowlistEntry`, `createAllowlistStore(filePath)` — the JSON-backed UPN list.                 |
| [src/index.ts](src/index.ts)                           | Public surface, including the `BootstrapAllowlistPolicy` type.                                                   |

`fromEnv` resolves the data dir, joins `header-allowlist.json` onto it, and
hands the resulting store to the provider. Swapping in another `AllowlistStore`
implementation — LDAP, a database table — means constructing `HeaderAuthProvider`
directly rather than through `fromEnv`.

```bash
cd packages/providers/header-auth
pnpm test          # vitest run — HeaderAuthProvider + allowlist store
```

No conformance suite here: the platform suites assume a password or token flow,
and this provider has neither.

---

## Configuration

The Selva app bundles this provider — set `SELVA_AUTH_PROVIDER=header` and pick a data/storage
provider separately. A custom runtime wiring it by hand does:
...
¹ Either `HEADER_AUTH_DATA_DIR` or `DATA_PATH` must be set — `fromEnv` throws if neither is.
...
The runtime resolves this via a one-shot **bootstrap allowlist policy**. On
every authed request, `@selvajs/selva` checks whether an instance admin already
exists; until one does, it asks the provider whether the incoming UPN/email
matches `BOOTSTRAP_INSTANCE_ADMIN_EMAIL`. A match auto-allowlists the user and
grants them every platform permission in the same request. The next visit is a
normal allowlisted login; the bootstrap window has closed.
...
Custom runtimes that wire `HeaderAuthProvider` themselves call
`provider.setBootstrapAllowlistPolicy(policy)` directly — see the
`BootstrapAllowlistPolicy` type for the contract, and pass `null` to clear it.
