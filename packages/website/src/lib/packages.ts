...
export interface PackageInfo {
	...
	featured?: boolean;
	/** Workspace folder, relative to the repo root. Its README is the package's reference doc. */
	dir: string;
	/** Published to npm. Workspace-internal packages have no npm page to link to. */
	published?: boolean;
	/** Site path to a generated Typedoc reference, e.g. '/docs/api/compute'. Only set where one is built. */
	apiDocsPath?: string;
}

const REPO = 'https://github.com/VektorNode/selva/blob/main';

/** Link to a package's README — the authoritative reference for its API. */
export function readmeUrl(pkg: PackageInfo): string {
	return `${REPO}/${pkg.dir}/README.md`;
}

/** Link to a package's npm page, or null when it isn't published. */
export function npmUrl(pkg: PackageInfo): string | null {
	return pkg.published ? `https://www.npmjs.com/package/${pkg.name}` : null;
}

/** Link to a package's generated API reference, or null when it doesn't have one. */
export function apiDocsUrl(pkg: PackageInfo): string | null {
	return pkg.apiDocsPath ?? null;
}
...
export const packages: PackageInfo[] = [
	...
	// ── Core libraries ────────────────────────────────────────────────────────
	{
		...
		badge: 'TypeScript',
		featured: true,
		apiDocsPath: '/docs/api/compute'
	},
	{
		name: '@selvajs/visualization',
		dir: 'packages/visualization',
		published: true,
	...
	},
	{
		...
		published: true,
		tagline: 'What outbound mail says',
		description:
	...
	},
	{
		...
		dir: 'packages/providers/supabase',
		published: true,
	...
	},
	{
		...
		dir: 'packages/providers/header-auth',
