...
- **B9 — `solve_metrics`: one unbatched INSERT per solve attempt. [P1]** `SupabaseSolveMetricSink.ts:18` — one `.insert()` per `record()`, no batching, no queue, no flush timer. Slider scrubbing at ~100 active users ⇒ hundreds of inserts/sec against the same Postgres serving auth. Fix: buffer + periodic batch flush behind `ISolveMetricSink`, or sample non-error metrics under load — keep failures unsampled, they feed alerting.
...
- **Q1 — The Supabase provider is untested in CI, and the deny-direction is dead everywhere. [P1]** `ctxIsolation` is declared and branched on in three conformance suites but **zero call sites pass `ctxIsolation: true`** — every cross-org/cross-user rejection test is dead code. `.github/workflows/test.yml` has no Supabase job; the conformance suites self-skip without creds.
  **Together with S2 this is the sharpest gap in the audit.** Fix: CI job with `supabase start` + migrations + `.env.test`; pass `ctxIsolation: true` from the Supabase conformance invocations; make `readEnv` **throw** (not skip) when `SUPABASE_URL` is set but the client can't construct; convert capability-absent silent returns to `it.skip` so skips are visible.
...
- **Q3 — Deletable/slimmable (~12). [P3]** .NET: `Selva.Drawing.Tests/SmokeTests.cs` (`Assert.True(true)` — delete); `Selva.Tests/JsonSchemaTests.cs` (strict subset of `SchemaValidatorTests` — delete); `SchemaMigratorTests:181` overlaps `:160` (merge). TS: `definitionStoreSuite.ts` "versioning scaffold" (delete); `storageProviderSuite.ts` `getPublicUrl` (keep one of three); `compute-server-encryption.test.ts` duplicate case (delete); `HeaderAuthProvider.test.ts` (slim); `updateCheck.test.ts`/`releaseChannel.test.ts` implementation-detail pins (fold/drop); `scenarios.test.ts` pure-rule matrices (relocate to `@selvajs/platform`); three identical `create populates createdBy/updatedBy` tests across suites (one shared helper).
