...
| ------ | ------ | -------------------------------------------------------------------------------------- | ------------------------------ |
| ☑      | **B9** | `solve_metrics`: unbatched INSERT per solve — **done**, buffered batch flush           | [§B](#b-scaling-roadmap)       |
...
- **B9 — `solve_metrics`: one unbatched INSERT per solve attempt. [P1 — DONE]** Was: one `.insert()` per `record()`, no batching, no queue, no flush timer, so slider scrubbing at ~100 active users meant hundreds of inserts/sec against the Postgres also serving auth. `SupabaseSolveMetricSink` now buffers rows and writes them in one multi-row insert, flushing on `maxBatchSize` (default 100) or `flushIntervalMs` (default 2000ms), whichever hits first. `record()` only buffers — it never awaits a write, so the hot path no longer pays a round-trip. A `maxBufferSize` cap (default 10k) drops oldest rows if the backend stalls; flushes are serialized so batches cannot interleave or double-write; `flush()` and `close()` are exposed for tests and shutdown drain. The `ISolveMetricSink` contract is unchanged (`record` still never throws). Batching is covered by `solve-metric-sink-batching.test.ts` (stack-free, fake client); the conformance suite gained an optional `flushSink` hook so buffering adapters round-trip correctly.
  **Not done:** sampling of non-error metrics under load, and no shutdown hook actually calls `close()` — there is no provider lifecycle/dispose seam today, so a hard kill still loses at most one partial batch (≤2s of telemetry). Wire `close()` in when a shutdown hook exists.
