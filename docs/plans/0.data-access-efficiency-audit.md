...
| ------ | ------ | ------------------------------------------------------------------------- | --------------------- |
| ☐      | **Q5** | Remaining gap: the core loop (upload → solve → render) still has zero E2E | [§Q](#q-test-quality) |
...
- **Q5 — Missing high-value cases. Items 1–6 + 8 shipped 2026-07-17; item 7 (E2E) is the remaining gap. [P1]**

  **Still open — 7. The product's core loop has zero E2E**: authed user → upload → set input → solve → binary geometry → render. Needs a compute server (real or faked at the transport seam) plus a browser harness, so it is its own project rather than a test to slot in. Related: `BinaryGeometryWriterTests.cs` verifies against a C#-reimplemented decoder, not the shipping JS parser — the format's two ends can still drift while both suites stay green (see [Not yet audited](#not-yet-audited)).
