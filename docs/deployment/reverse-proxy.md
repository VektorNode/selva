...
Open port 443 (and 80, for ACME + redirect) in your host's firewall — consult its docs, this varies per provider. Caddy issues and renews the TLS cert automatically. Update `.env`: `ORIGIN=https://selva.yourdomain.com`, and remove `ALLOW_INSECURE_COOKIES=true` if it was set for HTTP-only testing. Then `npm run restart`.
