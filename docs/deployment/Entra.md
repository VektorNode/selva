---
title: Entra SSO (oauth2-proxy + Caddy)
group: Deployment
order: 2
published: true
---
...
How to put a Selva (header-auth) deployment behind Microsoft Entra login using plain Caddy and oauth2-proxy.
...
Caddy is the security boundary — Selva must be unreachable except through it, or anyone can spoof the `SELVA-*` headers.

## Assumptions

- Ubuntu/Debian VM, plain Caddy already running as a systemd service.
- Selva already scaffolded (e.g. `~/selva`).
- Real domain with A record pointing at the VM, port 443 open.

Replace `[your-domain]` and `admin@corp.com` throughout.
