...
| Visibility fix          | [packages/ui/src/lib/schema/visibility-rules.ts](../../packages/ui/src/lib/schema/visibility-rules.ts)                                               | `evaluateVisibility` now honors static `item.visible === false` (was previously ignored: a pre-existing bug uncovered by this work) |
...
| Plugin-UI initial solve | [packages/plugin-ui/src/lib/composables/usePreviewState.svelte.ts](../../packages/plugin-ui/src/lib/composables/usePreviewState.svelte.ts)           | Initial-solve timeout reads live `state.values` (was using stale snapshot; a race condition fix)                                    |
...
- **For text inputs holding JSON**, the producer should `writeExternalValue({ value: JSON.stringify(payload) })`, keeping `values[paramId]` a string so the selva-app's `transformInputParameter` doesn't fight you. (Or improve `transformInputParameter` to stringify objects when `paramType === 'text'`: see "future improvements" below.)
...
- `RunnerComponent` is a lazy `() => import('./Runner.svelte')`, keeping the initial bundle small even with many producers.
