...
> `packages/schemas/fixtures/wire/` loaded by both `dotnet test` and `vitest`, with drift verified to
...
instead of a flat array; a `data:`-wrapped `parametersAdded` the UI read flat) surface only
