...
**Both ends of that invariant are atomic, and have to be.** `set()` takes real row locks: a plain `count`-then-`update` let two concurrent demotions each see the other as the survivor and both commit, leaving zero admins. Supabase takes check and write into one `selva.set_platform_permissions` RPC that locks the target row and then the surviving admins with `for update`; a single statement with an `exists` subquery is **not** enough, because under READ COMMITTED the subquery reads the statement's opening snapshot and every concurrent demotion sees the others as still-live. Local serializes the two through a mutex on `LocalUserDataStore.updatePermissionsGuarded`, which protects one process, the same boundary its load-once cache already draws. **A caller that counts and then calls `set` has reintroduced the race.** The mirror case at the other end is `claimFirstInstanceAdmin`, which grants only if no admin exists yet and re-checks under a lock: read-then-write there let two people signing in at the same moment on a fresh install both become permanent platform admins. Neither is reachable through the UI in normal use; both are reachable on a multi-instance deployment where requests are genuinely concurrent.
...
1. If the definition has `computeServerId` set **and** that server is in `serversVisibleTo(project.orgId)` → use that server. Lets a single heavy definition route to beefier hardware. A pin to a no-longer-visible server falls through (defensive: the admin may have un-shared the server since the pin was set).
...
**`private` means private, full stop**: from everyone who isn't an explicit project member, org leadership included. A member's personal R&D, a project an employee doesn't want their manager to see yet, a side project that happens to live in an org: all are valid reasons to want one, and org `admin`/`owner` role grants no access to any of them. Leadership's escape hatch is **Reclaim** (§5 `canReclaim`), an explicit, auditable action that adds them as co-owner. That audit trail is the load-bearing protection: the cost of "I need to access this" is one intentional step, not a silent default.
...
**Project-as-container model** (`autoJoinOnUpload=false`, the default), for enterprise/internal projects.
...
**Project-as-commons model** (`autoJoinOnUpload=true`), for user-generated content, commons publishing.
...
- **New definition** (`POST /api/definitions`): on container projects, requires `canEditDefinition` to pass preemptively, which in practice means project `owner`/`editor`. On commons projects, any authenticated user on the instance may create a new definition; `ownerId` is set to the uploader.
...
| `/api/v1/orgs/[orgId]/invites/[id]/resend`               | `POST`        | `manage_org_members`. Same tenancy check. Mints a replacement invite and revokes the original, so the superseded link stops working. Grants are copied from the original rather than re-derived: `canChangeOrgRole` ran at mint time, and re-running it here would let a since-demoted admin alter what the invite carries. 409 once accepted.    |
...
- **Sole-owner projects on offboarding:** removal **proceeds**, and the projects left without an owner are reported via `org_member.removed_orphaning_projects` (one event listing all of them). No silent reassignment, and no block.
  ...
  Blocking was the earlier rule, retired deliberately: its cost scales with how many projects the departing person owned, which is backwards. The most prolific people are the ones whose departure most needs to be clean, and a stalled offboarding leaves them in the org while someone works the backlog. An ownerless project is recoverable at leisure; a half-finished removal is a live account. **Reclaim** (§5) already adopts an ownerless project, so what was missing was never the recovery path, only the signal that recovery was needed.
...
| Bob (Acme `member`, no project membership) views and solves an Acme `org` project, then tries to edit its definition.                                          | **View/solve OK**: org visibility grants both to every org member. **Edit 403**: `canEditDefinition` needs an explicit project `owner`/`editor` role; org membership alone is view + solve, not edit.                                                                                                                                                                               |
...
| Marcus (Acme `admin`, no project memberships) opens `/projects`.                                                                                               | He sees `org` and `public` Acme projects (any org member's entitlement) but **not** private ones he isn't a member of (e.g., R&D Sandbox). A plain `member` such as Bob sees exactly the same set: org role changes nothing here. `private` means private from everyone without a membership, including org leadership. Reclaim is the explicit escalation path if he needs access. |
...
| Acme org member with no grant navigates to a `platform` project URL.                                                                                           | **403.** No grant exists for them.                                                                                                                                                                                                                                                                                                                                                  |
...
- Webhooks (events emit; dispatcher is later, slotting in as another `IEventSink` alongside the audit sink)
...
- **Multi-tenant SaaS mode**: self-service signup, public org creation, plan/billing, quota enforcement, past-due/read-only state. The data model and the `tenancy: 'multi'` switch already accommodate it; what's deferred is the user-facing flow and the gates around it. When this ships:
  ...
  - **The "user has zero orgs" state needs handling.** Today §9 says `actingOrgId` is required at every tenant-touching handler (400 if missing). Fresh SaaS signups legitimately have no orgs yet, so those routes need an allowlist (`POST /api/orgs`, profile, logout) and the rest should redirect to a "create or join an org" landing page rather than 400. Mechanical sweep, not architectural.
