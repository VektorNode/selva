...
3. **Delta filter.** Each vertex component is stored as the difference from the previous vertex, zigzag-mapped so small differences become small unsigned numbers. Welded meshes have spatially-local vertices, so the deltas cluster near zero. This is a PNG-style pre-filter and it exists purely to make the last step work better. A side effect worth knowing: the filter makes a mesh's vertex stream translation-invariant, so repeated parts (the same screw placed 500 times) become repeated byte runs that DEFLATE dedupes — instancing-like savings with no instancing in the format.
4. **Planar byte-split.** The filtered streams are rearranged into byte planes: all X deltas, then Y, then Z, low bytes before high bytes (v4). Near-zero deltas make the high planes runs of zeros; measured on welded grids, CAD scatters and instanced assemblies this shrinks the deflated wire 25–50% versus the interleaved v3 layout, at equal-or-better decode speed. Colors stay interleaved — planar loses on noisy per-channel data.
5. **Deflate.** The rearranged stream is compressed.
