...
`AssemblyVersion` / `FileVersion` stay numeric (`x.y.z.0`), since .NET rejects a suffix there, while `<Version>` / `<InformationalVersion>` (what yak and users see) carry the full `-beta.N`. Promoting to stable is just a normal release at the target `x.y.z`.
...
- **"You do not have permission"**: run `npm whoami`; check `@selvajs` org access.
