...
`Selva.gha` is the Grasshopper side of Selva. You use it to turn a definition into a web interface, and Rhino.Compute loads it to solve that definition once the app is deployed. One self-contained `.gha` — the web assets are embedded, so there is nothing else to install.

## The mental model

A Selva definition holds three kinds of thing, and nearly every component belongs to exactly one:
...
- **Inputs** are the `Get …` parameters. They are _contextual_ params: leave one unwired on the canvas and it takes its value from the web UI at solve time, then passes it downstream. Wire a source into it and the definition still solves sensibly inside Rhino.
...
The `Get …` inputs sit under **Params** rather than the Selva tab, because they are Grasshopper _parameters_, not components. They draw with a purple corner overlay to mark them as contextual.
...
| **Evaluate Schema** | Debug aid. Give it a schema and it prints a readable summary, the raw JSON, and the input/output counts.                                                                                                                     |

Details: [UI Builder](../plugin/ui-builder.md).

### Inputs — what the user drives

Contextual params, all under **Params → Util**. Drop one, wire its output into your logic, and you get a control in the web UI.
...
| **Get Image**              | Image upload          | PNG/JPEG/WEBP/SVG. Does no Rhino import — it carries the raw image downstream, usually into Drawing's Draw Image.                                                                                               |
...
| **Display From File** | Reads one back. Together the pair lets you mesh once and reuse the result instead of re-meshing every solve.                                                                                                                                |
...
The first four each produce a _File_ output. Wire that into a ContextBake and it becomes a download in the web app. All four take an optional sub-folder and `Key=Value` metadata.

Details: [File I/O](../plugin/file-io.md).

### Drawing — 2D sheets

A document-model drawing library: pages, frames, grids, views, dimensions, tables, title blocks and styles, rendered to SVG or PDF. It is the largest feature area, so it gets its own page.
...
Open the designer at the UI Bridge's URL, drag the input onto a control, and save. The schema now lives inside the `.gh`, and the definition is ready to deploy.
...
The repo README lists the install path for each OS.
