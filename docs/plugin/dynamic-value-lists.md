...
A normal dropdown has fixed options: you write them into a Grasshopper value list and they never change. A **dynamic** value list computes its options during the solve — from a file the user uploaded, from a database, from whatever the definition just calculated.

This is the one part of Selva that is genuinely a loop rather than a straight line, which is why it needs its own page. The confusing part is that it takes **two components plus a link you set in the UI builder**, and nothing on the canvas shows you that link.
...
Three pieces, and the link between the first and third is the one that isn't visible on the canvas:
...
Why is it a loop? Because the options themselves usually depend on other inputs. The user uploads a file, the definition reads the layer names out of it, those become the dropdown options, the user picks one, and the next solve uses that pick. That round-trip is exactly one solve long.

## Wiring it up

### 1. Drop a **Get Dynamic Value List**

_Params → Util._ Wire its output into whatever consumes the selection. Leave the input unwired for now.

This param is what the user sees as a dropdown. On its own it has no options — it emits an empty string until something populates it.
...
The quoted part is what the user sees; the part after `=` is what flows out of the Get param when that option is picked. Build these strings however you like — concatenate them from your data.
...
Same gesture as every other non-geometry output. This is what makes the UI Bridge notice it and put it in the schema.
...
This is the step people miss. Until you set it, the Set component runs, computes its options perfectly, and sends them nowhere. The component tells you so — it posts a remark on the canvas:
...
A Get param with no options yet is an empty dropdown, which is an awkward first impression. You can wire a source of `"name" = value` pair strings — a panel, or anything producing those strings — directly into the Get param on the canvas. Those become the starting options, and the first one is the default until the user picks something.

Computed options always win once they arrive. Think of the wired source as the fallback that makes the definition sensible when you open it in Rhino.

## Multi-select

Set the widget's display to **checklist** in the schema designer, and the param switches to list access — it emits every selected value instead of one.
...
- **Unknown values pass through.** Unlike the static value list, there is no authoritative option set to validate against — the options are recomputed every solve. If the selection doesn't match any current option, it flows downstream verbatim rather than being dropped.
