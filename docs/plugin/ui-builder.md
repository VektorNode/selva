...
Every Selva definition needs exactly one **UI Bridge** (Selva → UI). It links the Grasshopper canvas to the web interface.

## What the schema is

The **schema** describes your app's interface: which parameters are controls, what kind of control each one is, how they are grouped and ordered, and what the outputs are.
...
Its job depends on where it runs.
...
- After each solve, scans for **ContextBake** components and merges newly-wired outputs into the schema — or drops the ones you have unwired.

**On Rhino.Compute, at run time:** none of the above. It spots the headless environment, emits the embedded schema, and stops. No servers, no background tasks.
...
When an output never shows up in the designer, check for a missing ContextBake first.

## Evaluate Schema

A debugging component. Feed it the UI Bridge's `Schema` output and it prints a readable summary, the raw JSON, and the input and output counts. Reach for it when the designer shows something you didn't expect and you want to see what is actually stored.
