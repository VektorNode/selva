...
If nothing prints, the header never arrives. The proxy is not setting it: nginx needs `proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;` spelled out, while Caddy does it by default; `ADDRESS_HEADER` changes nothing until it does.
